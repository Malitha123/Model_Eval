...
def main_worker(gpu, ngpus, fold, args, cfg):
    ...
    print('get flops')
    dummy_inputs = torch.rand(64, 3, 32, 112,112)
...
def run_phase(phase, loader, model, optimizer, epoch, args, cfg, logger):
    ...
    correct_count_df.to_csv(f'./jobs/testing/{args.pretext_model_name}_correct_predicitions.csv',index=False)
    incorrect_count_df.to_csv(f'./jobs/testing/{args.pretext_model_name}_incorrect_predicitions.csv',index=False)
