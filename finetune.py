...
def run_phase(phase, loader, model, optimizer, epoch, args, cfg, logger):
    ...
    correct_count_df.to_csv(f'./jobs/csvs/{args.pretext_model_name}_full_fine_tune_correct_predicitions.csv',index=False)
    incorrect_count_df.to_csv(f'./jobs/csvs/{args.pretext_model_name}_full_fine_tune_incorrect_predicitions.csv',index=False)
